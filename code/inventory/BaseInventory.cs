...
public class BaseInventory : IBaseInventory
{
	...
	{
		Log.Info("BaseInv > SetActiveSlot");
		var ent = GetSlot( i );
		if ( Active == ent )
			return false;
	...
	{
		Log.Info("BaseInv > SetActive");
		if ( Active == ent ) return false;
		if ( !Contains( ent ) ) return false;

		if (Active is Weapon activeWeapon) {
			activeWeapon.ActiveEnd(Owner, false);
			if (ent is Weapon weaponToActivate) {
				weaponToActivate.ActiveStart(Owner);
			}
		}
