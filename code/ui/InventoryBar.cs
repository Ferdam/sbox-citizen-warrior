...
public class InventoryBar : Panel
{
	...
	{
		Log.Info("Inventory > SetActiveSlot()");
		var player = Local.Pawn as Player;
		...
		Log.Info(ent);
		if (inventory.Active is Weapon activeWeapon) {
			Log.Info("Active is Weapon");
			activeWeapon.ActiveEnd(player, false);
			if (ent is Weapon weaponToActivate) {
				Log.Info("Ent is Weapon");
				weaponToActivate.ActiveStart(player);
			}
		}
